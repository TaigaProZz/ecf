...
function AdminFeedback () {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get('ecf-node-serverr.vercel.app:${process.env.REACT_APP_3307}/feedback');
  ...
  const handleValidate = async (choice, feedbackId) => {
    if (choice === 'valider') {
      try {
        await axios.put(`ecf-node-serverr.vercel.app:${process.env.REACT_APP_3307}/feedback/${feedbackId}`, {isVerified: 1}); 
        fetchData();
  ...
  const handleHide = async (choice, feedbackId) => {
    if (choice === 'valider') {
      try {
        await axios.put(`ecf-node-serverr.vercel.app:${process.env.REACT_APP_3307}/feedback/${feedbackId}`, {isVerified: 0});
        fetchData();
  ...
  const handleDelete = async (choice, feedbackId) => {
    if (choice === 'valider') {
      try {
        await axios.delete(`ecf-node-serverr.vercel.app:${process.env.REACT_APP_3307}/feedback/${feedbackId}`);
        fetchData();
  ...
  const addFeedback = async (newFeedback) => {
    ...
    try {
      await axios.post(`ecf-node-serverr.vercel.app:${process.env.REACT_APP_3307}/feedback`, newFeedback);
