...
function AdminSecondHand () {
  ...
  const fetchData = async () => {
    const response = await axios.get(`ecf-node-serverr.vercel.app:${process.env.API_PORT}/car`);
  ...
  const addCar = async (car) => {
    ...
    try {
      const response = await axios.post(`ecf-node-serverr.vercel.app:${process.env.API_PORT}/car`, formData);
  ...
  const deleteCar = async (choice, carId) => {
    if(choice === 'valider') {
      try {
        await axios.delete(`ecf-node-serverr.vercel.app:${process.env.API_PORT}/car/${carId}`);
        fetchData();
