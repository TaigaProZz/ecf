...
function AdminSecondHand () {
  ...
  const fetchData = async () => {
    const response = await axios.get(`ecf-node-serv.vercel.app:3307/car`);
  ...
  const addCar = async (car) => {
    ...
    try {
      const response = await axios.post(`ecf-node-serv.vercel.app:3307/car`, formData);
  ...
  const deleteCar = async (choice, carId) => {
    if(choice === 'valider') {
      try {
        await axios.delete(`ecf-node-serv.vercel.app:3307/car/${carId}`);
        fetchData();
