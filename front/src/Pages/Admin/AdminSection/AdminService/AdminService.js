...
function AdminServices () {
  ...
  const addService = async (service) => {
    ...
    try {
      await axios.post(`ecf-node-serverr.vercel.app:3307/service`, { services: service });
      fetchData();
    } catch (error) {
  ...
  const deleteService = async (id) => { 
    try {
      await axios.delete(`ecf-node-serverr.vercel.app:3307/service/` + id);
      fetchData();
    } catch (error) {
  ...
  const updateService = async (service, id) => {
    ...
    try {
      await axios.put(`ecf-node-serverr.vercel.app:3307/service/` + id, {services: service});
      fetchData();
    } catch (error) {
