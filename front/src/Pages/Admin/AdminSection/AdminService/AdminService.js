...
function AdminServices () {
  ...
  const fetchData = async () => {
    const response = await axios.get(`ecf-node-serverr.vercel.app:${process.env.REACT_APP_3307}/service`);
  ...
  const addService = async (service) => {
    ...
    try {
      await axios.post(`ecf-node-serverr.vercel.app:${process.env.REACT_APP_3307}/service`, { services: service });
      fetchData();
    } catch (error) {
  ...
  const deleteService = async (id) => { 
    try {
      await axios.delete(`ecf-node-serverr.vercel.app:${process.env.REACT_APP_3307}/service/` + id);
      fetchData();
    } catch (error) {
  ...
  const updateService = async (service, id) => {
    ...
    try {
      await axios.put(`ecf-node-serverr.vercel.app:${process.env.REACT_APP_3307}/service/` + id, {services: service});
      fetchData();
    } catch (error) {
