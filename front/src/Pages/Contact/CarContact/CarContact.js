...
function Contact() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`ecf-node-serverr.vercel.app:${process.env.REACT_APP_3307}/car/${params.id}`);
  ...
  const sendData = async (subject, name, phone, email, message) => {
    try {
      await axios.post(`ecf-node-serverr.vercel.app:${process.env.REACT_APP_3307}/contact`, {
        subject: subject,
        name: name,
        phone: phone,
        email: email,
