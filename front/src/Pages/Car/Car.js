...
function Car() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const carResponse = await axios.get(`ecf-node-serverr.vercel.app:3307/car/${params.id}`);
        const car = carResponse.data;
        setElement(car[0]);

        const imageResponse = await axios.get(`ecf-node-serverr.vercel.app:3307/carimage/${params.id}`);
